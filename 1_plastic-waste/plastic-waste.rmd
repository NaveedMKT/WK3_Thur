...
Did you expect this result? You might consider doing some research on Trinidad and Tobago to see why plastic waste per capita is so high there, or whether this is a data error. Not a data error.
...
It is because the aesthetics function while using color groups and colors each category within the variable but for the alpha function describes the transparency. Aesthetics applies color for only the data but the alpha applies it to the whole thing so it is used in the plotting geom rather than under the aesthetics.
...
The distribution of the various data points is way more apparent in violin plots as compared to box plots. In the box plots, you are shown the mean and the interquartile range but not in the violin plots.
```{r}
ggplot(data = plastic_waste, 
       mapping = aes(x = continent, 
                     y = plastic_waste_per_cap)) +
